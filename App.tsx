...
const App: React.FC = () => {
  ...
  const renderNavbar = () => (
    <nav className="bg-white border-b border-slate-200 sticky top-0 z-40">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between h-16">
          ...
          <div className="hidden md:flex items-center space-x-8">
            <button onClick={() => handleNavClick('HOME')} className={`text-sm font-medium ${view === 'HOME' ? 'text-blue-600' : 'text-slate-500 hover:text-slate-900'}`}>Home</button>
            <button onClick={() => handleNavClick('BROWSE')} className={`text-sm font-medium ${view === 'BROWSE' ? 'text-blue-600' : 'text-slate-500 hover:text-slate-900'}`}>Find Expert</button>
            <button onClick={() => handleNavClick('DASHBOARD')} className={`text-sm font-medium ${view === 'DASHBOARD' ? 'text-blue-600' : 'text-slate-500 hover:text-slate-900'}`}>Dashboard</button>
          </div>

          <div className="flex items-center space-x-4">
             <div className="hidden lg:flex text-slate-500 text-sm gap-2">
                 <span>EN</span>|<span>FR</span>|<span>HA</span>
             </div>
      ...
      {/* Mobile Menu Overlay */}
      {isMobileMenuOpen && (
        <div className="md:hidden bg-white border-t border-slate-100 py-2 px-4 space-y-2 shadow-lg absolute w-full left-0 animate-in slide-in-from-top-2 duration-200">
          <button onClick={() => handleNavClick('HOME')} className={`block w-full text-left py-3 px-3 rounded-md text-base font-medium ${view === 'HOME' ? 'bg-blue-50 text-blue-600' : 'text-slate-600'}`}>Home</button>
          <button onClick={() => handleNavClick('BROWSE')} className={`block w-full text-left py-3 px-3 rounded-md text-base font-medium ${view === 'BROWSE' ? 'bg-blue-50 text-blue-600' : 'text-slate-600'}`}>Find Expert</button>
          <button onClick={() => handleNavClick('DASHBOARD')} className={`block w-full text-left py-3 px-3 rounded-md text-base font-medium ${view === 'DASHBOARD' ? 'bg-blue-50 text-blue-600' : 'text-slate-600'}`}>Dashboard</button>
          <div className="border-t border-slate-100 pt-3 mt-2">
             <div className="flex justify-center gap-4 text-sm text-slate-500 mb-4">
                 <span>EN</span><span>FR</span><span>HA</span>
             </div>
            <button className="w-full bg-slate-900 text-white px-4 py-3 rounded-lg text-sm font-medium">Sign In</button>
          </div>
        </div>
      )}
    </nav>
          ...
          <main className="mt-8 mx-auto max-w-7xl sm:mt-12 md:mt-16 lg:mt-20 xl:mt-28">
            <div className="text-center lg:text-left">
              <h1 className="text-4xl tracking-tight font-extrabold text-slate-900 sm:text-5xl md:text-6xl">
                <span className="block xl:inline">Connect with Global</span>{' '}
                <span className="block text-blue-600 xl:inline">Verified Experts</span>
              </h1>
              <p className="mt-3 text-base text-slate-500 sm:mt-5 sm:text-lg sm:max-w-xl sm:mx-auto md:mt-5 md:text-xl lg:mx-0">
                {TAGLINE} Access professionals in Tech, Health, Law, and more instantly. Powered by AI matching to find your perfect fit.
              </p>
  ...
  const renderProfileView = () => {
    ...
    return (
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
        <button onClick={() => handleNavClick('BROWSE')} className="mb-6 flex items-center text-slate-500 hover:text-blue-600 transition-colors">
  ...
  const renderDashboard = () => (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
       <h2 className="text-2xl sm:text-3xl font-bold text-slate-900 mb-8">My Dashboard</h2>
        ...
        {view === 'HOME' && (
          <>
            ...
            <div className="py-12 bg-white">
              <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <h2 className="text-2xl font-bold text-slate-900 mb-6">Featured Experts</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
            ...
            <div>
              ...
              <ul className="space-y-2 text-slate-400 text-sm">
                <li onClick={() => handleNavClick('BROWSE')} className="cursor-pointer hover:text-white transition-colors">Browse Experts</li>
